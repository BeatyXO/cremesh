...
export async function connectWallet(kind:'injected'|'browser'){if(kind==='browser') throw new Error('Browser-generated wallets are disabled. Use an injected wallet.');if(typeof window==='undefined'||!window.ethereum) throw new Error('No injected EIP-1193 wallet detected. Install MetaMask, Rabby, or another browser wallet.');const accounts=await window.ethereum.request({method:'eth_requestAccounts'}) as string[];return accounts[0];}
