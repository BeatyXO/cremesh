...
export function getWriteClient(address:string){return createClient({chain, account:address});}
export async function connectWallet(kind:'injected'|'browser'){if(kind==='injected'){if(typeof window==='undefined'||!window.ethereum) throw new Error('No injected EIP-1193 wallet detected. Use a browser wallet instead.');const accounts=await window.ethereum.request({method:'eth_requestAccounts'}) as string[];return accounts[0];} let key=localStorage.getItem('credentialmesh.browser-key'); if(key===null){key=generatePrivateKey();localStorage.setItem('credentialmesh.browser-key',key);localStorage.setItem('credentialmesh.browser-wallet-warning','acknowledged');} return createAccount(key).address;}
